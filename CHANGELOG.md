...
### Added
- **Handler `message:audio` en Telegram** (`src/channels/telegram.ts`): soporte para archivos de audio enviados como adjuntos (MP3, M4A, WAV, OGG, FLAC, WebM), complementando el handler `message:voice` existente. Reusa el mismo `Transcriber` y el mismo prefijo `[Mensaje de voz, ...]` para que el skill `voice-messages` se active por trigger; cuando Telegram envía `file_name`, se incluye como hint: `[Mensaje de voz, 2m10s] (archivo: grabacion.mp3) ...`. Nuevo helper `extFromAudio(fileName, mimeType)` que acepta el audio si **o** la extensión del archivo **o** el MIME type identifican un formato soportado (retorna `null` cuando ninguno matchea → rechazo con mensaje amigable). Crítico: Telegram marca `Audio.mime_type` como opcional, así que el fallback por filename evita rechazar uploads válidos de clientes que sólo mandan el nombre.
- **Retry con backoff en `GroqTranscriber`** (`src/transcription/transcriber.ts`): el SDK Groq se configura ahora con `maxRetries: 0` para que el loop propio maneje la política completa. 3 reintentos con backoff exponencial capeado en 8s + jitter. Retryable: `408`, `409`, `429`, `500`, `502`, `503`, `504`, **más errores sin HTTP status** (network errors, timeouts, socket reset, `AbortError`) — emulando la cobertura que el SDK ofrecía internamente. En `429` se respeta el header `Retry-After` via helper `readRetryAfter()` que detecta si `err.headers` es un `Headers` object (usa `.get()`) o un record plano (defensivo para futuras versiones del SDK).
- **Scheduler job `uploads-cleanup`** (`src/scheduler/uploads-cleanup.ts`): nueva tarea cron que elimina archivos en `./data/uploads/` más viejos que `UPLOADS_CLEANUP_RETENTION_DAYS` (default 30). Corre silenciosamente (solo logs, sin notificación Telegram) cada `UPLOADS_CLEANUP_INTERVAL_HOURS` (default 24). El intervalo sólo acepta divisores de 24 (`{1, 2, 3, 4, 6, 8, 12, 24}`) para garantizar que el cron expression `0 */N * * *` represente exactamente "cada N horas"; valores inválidos se clamp-ean a 24 con warning al seedeo. Tipo `uploads-cleanup` agregado a `TaskType` en `src/scheduler/types.ts` + case en `src/scheduler/scheduler-manager.ts` + seedeo en `src/index.ts`.
- **Nuevas variables de entorno** (`src/config.ts` + `.env.example`): `UPLOADS_CLEANUP_ENABLED` (default `true`), `UPLOADS_CLEANUP_INTERVAL_HOURS` (default 24, con restricción de divisores documentada), `UPLOADS_CLEANUP_RETENTION_DAYS` (default 30). Nuevo namespace `config.uploadsCleanup`.
- **Skill `voice-messages.md` enriquecido**: decision tree por duración del audio (heurístico: si el prefijo contiene `m` → audio largo). Audios cortos (<60s) se responden natural/conversacional. Audios largos (≥60s) generan respuesta estructurada con resumen de 2-3 bullets, tono detectado (`urgente`/`relajado`/`preocupado`/`entusiasta`/`neutral`) y respuesta a cada tema. Sección explícita de detección de tono por señales léxicas (inferencia solo por texto transcrito). Incluye 2 ejemplos concretos para anclar el comportamiento.

### Changed
- **`CLAUDE.md`**: Groq Whisper agregado al Tech Stack; `src/transcription/` agregado al árbol de arquitectura; nota explícita de que el handler de Telegram cubre texto, foto, documento, voz **y audio**.

### Added
- **Auto-link worktree ↔ backlog en `git_worktree` create** (`src/tools/built-in/git-worktree.ts`): la acción `create` ahora acepta el parámetro opcional `backlog_item_id`. Cuando se pasa, el tool actualiza atómicamente el item con `branch_name`, `worktree_path` y `status='in_progress'`. **WHERE restringido a `status='open'`**: el UPDATE rechaza items en `in_progress`/`pr_created`/`merged`/`dismissed` para no pisar history (ej. un item ya mergeado no debe re-abrirse si el agente reusa un id viejo por error). El error devuelto distingue entre "item no existe" vs "item existe pero no está open". **Atomicidad real**: si el UPDATE no afecta filas (id inválido, item cerrado, id de otro user) o tira error, el tool hace rollback del worktree (`git worktree remove --force` + `git branch -D`) y devuelve error. Así nunca queda un worktree físico sin link cuando el caller pidió link. El factory `createGitWorktreeTool(codebaseConfig, db?)` recibe ahora la DB opcional; `src/index.ts` la inyecta.
- **Scheduler nuevo `worktree-reconciler`** (`src/scheduler/worktree-reconciler.ts`): job cron que corre cada `WORKFLOW_RECONCILER_INTERVAL_MINUTES` (default 30) con tres pasadas:
  - **Pasada A — huérfanos físicos**: detecta worktrees dentro de `<root>/<worktreesDir>` que no tengan **ninguna** fila en `backlog_items` apuntándolos (cualquier status). Esto preserva items en `merged`/`dismissed` con `worktree_path` poblado (cuando el cleanup fue `skipped_dirty` o `WORKFLOW_AUTO_CLEANUP_WORKTREE=false`) para que el usuario pueda inspeccionar manualmente. Antes de eliminar requiere **cuatro** condiciones: (1) working tree limpio, (2) **sin commits unpushed** (comparando contra el upstream configurado, o `origin/<defaultBranch>` si la rama nunca fue pusheada), (3) sin PR abierto para la rama (`gh pr list --head ...`), (4) antigüedad > `WORKFLOW_RECONCILER_ORPHAN_AGE_MINUTES` (default 120). Si alguna falla, loguea y salta. Fail-safes: si `gh` falla (no instalado / no auth / offline) trata como "tiene PR abierto"; si `git rev-list` falla trata como "tiene unpushed" — siempre preservando el worktree.
  - **Pasada B — referencias fantasma**: backlog items con `worktree_path` que ya no existe en disco. Si la branch tampoco existe localmente, se limpian ambos campos; si la branch todavía existe, solo se limpia `worktree_path`.
  - **Pasada C — ramas huérfanas**: backlog items con `worktree_path=NULL` pero `branch_name != NULL` (dejadas así por `github-pr-monitor` cuando `git branch -D` falló durante el cleanup, típicamente porque la rama aún no estaba fully merged). El reconciler reintenta: si la rama existe y se puede borrar ahora → `git branch -D` + clear branch_name; si la rama ya no existe localmente → solo clear branch_name (stale reference); si sigue fallando → loguea y reintenta la próxima corrida.
  - Notificación Telegram único al final con el resumen, sólo si hubo cambios.
  - Seedeo en `src/index.ts` cuando `WORKFLOW_ENABLED=true` + `CODEBASE_ENABLED=true` + `WORKFLOW_RECONCILER_ENABLED=true`.
- **Nuevas variables de entorno** (en `src/config.ts` + `.env.example`): `WORKFLOW_RECONCILER_ENABLED` (default `true`), `WORKFLOW_RECONCILER_INTERVAL_MINUTES` (30), `WORKFLOW_RECONCILER_ORPHAN_AGE_MINUTES` (120). Nuevo namespace `config.workflowReconciler`.
- **Tipo de task `worktree-reconciler`**: agregado a `TaskType` en `src/scheduler/types.ts` y handler en `src/scheduler/scheduler-manager.ts`.

### Changed
- **Prompt del scheduler code-review autonomous-fix** (`src/scheduler/code-review.ts:275-282`): simplificado. Antes el agente debía llamar `manage_backlog update_item status=in_progress` y luego `git_worktree create` por separado, y "al final actualizar el backlog con la PR URL". Ahora el prompt le indica pasar `backlog_item_id` a ambos tools para que los vínculos se registren atómicamente (status=in_progress al crear worktree, status=pr_created al crear PR). Elimina los pasos manuales redundantes que eran la causa principal de worktrees huérfanos cuando el flujo se interrumpía a mitad de camino.
- **`github-pr-monitor` limpia campos stale tras cleanup — preservando branches vivas** (`src/scheduler/github-pr-monitor.ts`): `CleanupOutcome` ahora incluye `branchDeleted: boolean` en los variants `removed` y `skipped_missing`. El UPDATE final usa tres statements: (1) si el worktree físico desapareció Y `git branch -D` fue exitoso → nullea `worktree_path` + `branch_name`; (2) si el worktree desapareció pero `git branch -D` falló (rama no fully merged) → nullea solo `worktree_path`, preserva `branch_name` como único pointer para cleanup manual; (3) para `skipped_dirty` / `error` → solo cambia status, preserva ambos campos. La notificación Telegram refleja si la rama quedó viva con un aviso.

### Fixed
- **Worktrees huérfanos por orquestación incompleta**: si el workflow autónomo se cortaba entre "crear worktree" y "crear PR", el item quedaba sin `branch_name`/`worktree_path` (o directamente sin vincular), y el `github-pr-monitor` — que sólo busca `status='pr_created'` — nunca lo veía ni limpiaba. Ahora el vínculo se establece atómicamente al crear el worktree, y el nuevo reconciler captura cualquier caso edge residual (incluyendo worktrees creados manualmente fuera del flujo).
